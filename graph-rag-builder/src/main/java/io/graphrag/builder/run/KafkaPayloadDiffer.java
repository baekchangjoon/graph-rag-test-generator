...
final class KafkaPayloadDiffer {
     ...
     * <p>규칙:
     * <ul>
     *   <li>textual 최상위 필드의 값이 emission1과 emission2에서 다르면 비결정으로 판단.</li>
     *   <li>그 값이 {@code inputDerivedValues}(입력 유래 치환 대상)에 포함되면 <b>제외</b>
     *       (REQ-010 불변 — 입력 유래 값은 절대 비결정으로 표시 금지).</li>
     *   <li>payload1과 payload2 양쪽 값을 반환 집합에 추가한다(Generator가 값-기반으로 조회).</li>
     ...
     * @return 비결정으로 판단된 값 집합 (두 발행의 값 모두 포함)
     */
