 ...
 * 인덱서가 산출(static 분석만 — DB 미기동)하고 러너/합성기가 소비한다. 런타임에서만 알 수 있는
 * 백업 테이블명/PK·name 컬럼은 여기 두지 않고 러너가 {@code TableSchema}로 해석한다.
 *
...
public record FormFieldBinding(
    ...
    public static FormFieldBinding scalar(String field, String javaType) {
        return new FormFieldBinding(field, javaType, Kind.SCALAR, null, null, null);
    }

    public static FormFieldBinding reference(String field, String javaType, String refEntityFqn,
                                             String joinColumn) {
        return new FormFieldBinding(field, javaType, Kind.REFERENCE, refEntityFqn, joinColumn, null);
    }

    public static FormFieldBinding nested(String field, String javaType, String nestedTypeFqn) {
        return new FormFieldBinding(field, javaType, Kind.NESTED, null, null, nestedTypeFqn);
    }
}
