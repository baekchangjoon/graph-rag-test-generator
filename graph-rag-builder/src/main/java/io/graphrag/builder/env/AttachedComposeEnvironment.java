...
public final class AttachedComposeEnvironment implements ExplorationEnvironment {
    ...
    // sleuth 모드에서만 non-null. runAttached가 만들어 주입하며 이 환경이 생명주기를 소유(close 시 stop)한다.
    // EgressCollector.forMode(env)가 이를 집어 otel(otlpReceiver)과 동일한 egress 파이프라인을 탄다.
    private final io.graphrag.builder.capture.zipkin.ZipkinSpanReceiver zipkinReceiver;
    ...
    public AttachedComposeEnvironment(Config config, DbConfig.Type dbType) {
        this(config, dbType, null, null, null);
    }

    public AttachedComposeEnvironment(Config config, DbConfig.Type dbType,
                                      io.graphrag.builder.capture.otlp.OtlpTraceReceiver otlpReceiver) {
        this(config, dbType, otlpReceiver, null, null);
    }

    public AttachedComposeEnvironment(Config config, DbConfig.Type dbType,
                                      io.graphrag.builder.capture.otlp.OtlpTraceReceiver otlpReceiver,
                                      HttpCaptureServer httpCapture) {
        this(config, dbType, otlpReceiver, httpCapture, null);
    }

    public AttachedComposeEnvironment(Config config, DbConfig.Type dbType,
                                      io.graphrag.builder.capture.otlp.OtlpTraceReceiver otlpReceiver,
                                      HttpCaptureServer httpCapture,
                                      io.graphrag.builder.capture.zipkin.ZipkinSpanReceiver zipkinReceiver) {
        this.config = config;
        this.dbType = dbType;
        this.otlpReceiver = otlpReceiver;
        this.httpCapture = httpCapture;
        this.zipkinReceiver = zipkinReceiver;
    }
    ...
    @Override public io.graphrag.builder.capture.zipkin.ZipkinSpanReceiver zipkinReceiver() { return zipkinReceiver; }
