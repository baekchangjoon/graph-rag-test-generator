...
### 재측정 (2026-09-02, 도구 main 기준): 153/253 (60.5%)

동일 jar(sha256 아래 표와 동일)에 현재 도구로 전 엔드포인트를 재탐색한 결과 **153/253
(60.5%)** — 2026-06-16 기록(145) 대비 +8 분기. 산출: 24 엔드포인트, 123 paths, SQL 343건,
스키마 8테이블. 조건: `--budget-requests 60 --trace-mode none --db-service postgres`,
`--sut-env SPRING_PROFILES_ACTIVE=postgres`(이게 없으면 스키마 init이 돌지 않아 전 요청
500 — 측정 무효), `GRB_SUT_BOOT_TIMEOUT_SECONDS=300`(아래 Phase A 절의 "PT1M30S 부팅
실패" 3연속과 같은 증상을 이 env로 해소). 2026-06 측정 러너(`.work/`)의 플래그와 완전
동일 조건임은 보장하지 못하므로 방향성 비교(현행 도구 실측값)로 읽는다.
...
petclinic의 잔여 145/253(57.3%)은 이 절 작성 시점(2026-07)의 상태다(2026-09-02 재측정은
153 — 위 "재측정" 소절) — 잔여 108개 분기가
