...
> **주의(2026-09-01).** 이 문서의 "Neo4j 클러스터" 서술은 초기 구상이다. 실제 채택은 파티션 샤드
> **파일 스토어**이고 Neo4j는 보류됐다 — [decisions/graph-store-phase6](decisions/graph-store-phase6.md).
> 아래 Neo4j 언급은 "그래프 저장소"로 읽되, 대규모 전략의 골격(파티션·증분)은 유효하다.
